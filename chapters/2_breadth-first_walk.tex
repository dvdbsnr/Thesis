...
\begin{proof} \label{P: formula an}
	...
	we see that
	\begin{equation}
	\Prob(\text{$(i,j)$ opens in $[s, s+ds]$}) = \Prob(\text{$(i,j)$ opens})\Prob(\text{It happens in $[s, s+ds]$}) = pds.
	\end{equation}
	By the definition of conditional probability
	\begin{equation}
	\begin{aligned}
	\Prob( \text{$(i,j)$ opens in $[s, s+ds]$} \cond \text{$(i,j)$ did not open in $[\floor{s}, s]$} )
	&= \frac{\Prob( \text{$(i,j)$ opens in $[s, s+ds]$})}{\Prob(\text{$(i,j)$ did not open in $[\floor{s}, s]$})} \\
...
\begin{proof} \label{P: limit An}
	...
	\begin{aligned}
	&|\an{s} - \amn{s}| \\
	&\quad = |(n - s - \Zetan{\ceil{s}} - \Zn{s}) \left( \ps - p(n) \right)| \\
	&\quad = |p(n)(n - s - \Zetan{\ceil{s}} - \Zn{s}) \left( \frac{1 - 1 + (s - \floor{s})p(n)}{1-(s - \floor{s})p(n)} \right)| \\
	&\quad = \underbrace{\frac{p(n)n - p(n)(s + \Zetan{\ceil{s}} + \Zn{s})}{1-(s - \floor{s})p(n)}}_{\xrightarrow{n \rightarrow \infty} 1} 
		\underbrace{(s - \floor{s})p(n)}_{=\BigO{n^{-1}}} \\
	&= \BigO{n^{-1}}.
	\end{aligned}
	\end{equation}
    ...
    The left-hand side of this inequality differs from expression needed for \eqref{E: limit An} by the term $\frac{s^2t}{2\n{4}{3}}$.
    ...
    \end{aligned}
    \end{equation}
    We note that $\BigO{\n{-2}{3}} \rightarrow_p 0$ as $n \rightarrow \infty$ and see that we can establish the Lemma by proving
