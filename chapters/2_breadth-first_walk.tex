...
\end{equation}
\todo{Remove equation? Unnecessary?}
\todo{Fix \text in braces: Space \; or \quad}
...
$(z(i), \; 1 \leq i \leq n)$
by
\begin{equation} \label{E: def bf-walk z+c}
\begin{aligned}
z(0) &:= 0, \\
...
\end{figure} 
\todo{Fix random walk: Error around node 9!}
\todo{Fix centering of figure captions}
\todo{Add lines indicating end of components in fig_bf-walk}
\todo{Add more vertex labels in original ordering?}
...
\todo{This sentence is a test for labeling of align-equs. Fix!}
Now we can provide a definition of the breadth-first walk equivalent to \eqref{E: def bf-walk z+c}:
\begin{equation}  \label{E: def bf-walk N+zeta}
\begin{aligned}
z(0) &:= 0, \\
...
\begin{proof}[Proof of Theorem \ref{T: convergence Zn}]
	...
	\todo{Martingales for/in?}
	\todo{Martingale = F-local martingale?}
