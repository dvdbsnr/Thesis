...
\begin{proof} \label{P: decomp Mn}
	...
	\end{equation}
	and to prove \eqref{decomp Mn} it suffices to show that
	$[M_n] - B_n$
	is a martingale.
	...
	&= \Zn{t} - \An{t}
	\end{align*}
	which is a martingale from Lemma \eqref{L: decomp Zn}.
...
\begin{proof} \label{P: formula an}
	...
	At time $i-1+\Uij$, the edge $(i,j)$ will open with probability $p(n)$ and
	...
	$P(\text{A new edge appears in $[s, s+ds]$} \cond \SigmaAlgebra_s)$.
	...
	\begin{equation}
	P(\NewEdge{[s,s+ds]} \cond \SigmaAlgebra_s^k)
	= \sum_{j \neq j_1, \dots j_k} P( \text{$(i,j)$ opens in $[s, s+ds]$} \cond \SigmaAlgebra_s^k ) + \Smallo{ds}.
	\end{equation}
	...
	Since the opening itself, happening with probability $p(n)$,
	and the uniformly distributed time of the event in $[i-1, i]$ are independent,
	we see that
	\begin{equation}
	P(\text{$(i,j)$ opens in $[s, s+ds]$}) = P(\text{$(i,j)$ opens})P(\text{It happens in $[s, s+ds]$}) = pds.
	\end{equation}
	By the definition of conditional probability
	\begin{equation}
	\begin{aligned}
	P( \text{$(i,j)$ opens in $[s, s+ds]$} \cond \text{$(i,j)$ did not open in $[\floor{s}, s]$} )
	&= \frac{P( \text{$(i,j)$ opens in $[s, s+ds]$})}{P(\text{$(i,j)$ did not open in $[\floor{s}, s]$})} \\
	&= \frac{pds}{1-p(s-\floor{s})},
	\end{aligned}
	\end{equation}
...
\end{proof}
\fxfatal{Add proof of an formula!}
...
\begin{proof} \label{P: limit An}
	...
	\end{align*}
	Using \eqref{L: zeta(i)}, the following inequalities hold for sufficiently large $n$:
    ...
    &\longrightarrow_p 0 
    \end{align*}
    will suffice to prove \eqref{E: limit An}.
    ...
    Using \eqref{E: (27)} and \eqref{L: zeta(i)} we obtain
    \begin{align*}
    ...
    This and \eqref{E: exp Mn} lead to a bound for large $n$:
    ...
    \end{equation}
    which proves \eqref{E: stoch bounded}.
\end{proof}
...
\begin{proof} \label{P: limit Bn}
	From its definition in Lemma \eqref{L: decomp Mn}, $\Bn{s} = \An{s} + s$, which allows us to rewrite \eqref{E: limit Bn} as
	...
	&\longrightarrow_p 0,
	\end{align*}
	as $n \rightarrow \infty$ from Lemma \eqref{L: limit An}. This gives \eqref{E: limit Bn An + c} and completes the proof.
\end{proof}
...
\begin{proof} \label{P: limit Mn}
	As previously discussed in the proof of Lemma \eqref{L: decomp Mn}, the jumps of $M_n$ are exactly the jumps of $Z_n$ and thus have size $1$. The lemma follows immediately.
\end{proof}
