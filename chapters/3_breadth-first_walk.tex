...
\begin{equation}
\Ni{i} := 
\left\lbrace v \in \{1, \dots, n\} \backslash \{ v_1,\dots, v_i \} 
\; | \; 
\left(v_j, v\right) \in S \; \text{for some} \; 1 \leq j \leq i \right\rbrace
\end{equation}
...
At step $1$, we traverse vertex $v_1$ and mark it as explored.
We search for neighbours of $v_1$, and mark these as discovered.
...
and \eqref{E: zeta-1}, the index of the component containing $v_i$.
\fxnote{This sentence is a test for labelling of align-equs. Fix!}
...
so $v_i \in \Ni{i-1}$.
Going from $i-1$ to $i$,
$\Ni{*}$ increases by the number of new neighbours of $v_i$
...
of length
$k = i-1 + |\Ni{i-1}|$.
The discrete walk now adds the children of $v_i$,
...
\begin{proof} \label{P: formula an}
	...
	That is, all vertices not yet explored or discovered, and excluding $v_i$ itself.
