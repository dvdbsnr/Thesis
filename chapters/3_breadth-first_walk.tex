...
In Figure~\ref{F: bf-walk discrete} we see this discrete walk applied to the components of the sample graph.
...
We verify the equivalence by induction, showing that for $i \geq 1$ increments of both functions are equal.
We have
\begin{equation} \label{E: equality z z*}
\begin{aligned} 
&\hspace{33pt} z(i) - z(i-1) = z^*(i)  -z^*(i-1) \\
&\iff \hspace{33pt} c(i) - 1  = |\Ni{i}| - \zeta^{-1}(i) - |\Ni{i-1}| + \zeta^{-1}(i-1) \\
&\iff \hspace{8pt} |\Ni{i}| - |\Ni{i-1}| = c(i) + \zeta^{-1}(i) - \zeta^{-1}(i-1) - 1.
\end{aligned}
\end{equation}
...
The newly defined continuous time walk adds those vertices at uniformly random times in $(i-1, i)$.
See Figure~\ref{F: bf-walk cont} for a sample component and the walk $Z$ on its vertices.

Note that, since jumps are now happening at random times during their respective time interval, 
...
\begin{proof} \label{P: formula an}
	...
	\begin{equation*}
	\Prob\left(\NewEdge{[s,s+ds]} \cond \SigmaAlgebra_s^k\right)
	= \sum_{j \neq j_1, \dots j_k} \Prob\left( \text{$(i,j)$ opens in $[s, s+ds]$} \cond \SigmaAlgebra_s^k \right) + \Smallo{ds}.
	\end{equation*}
	...
	\begin{equation*}
	\begin{aligned}
	&\Prob\left(\text{$(i,j)$ opens in $[s, s+ds]$}\right) \\
	&\quad = \Prob\left(\text{$(i,j)$ opens}\right)\Prob\left(\text{It happens in $[s, s+ds]$}\right) \\
	&\quad = \p ds.
	\end{aligned}
	\end{equation*}
	...
	\begin{equation*}
	\begin{aligned}
	&\Prob\left( \text{$(i,j)$ opens in $[s, s+ds]$} \cond \text{$(i,j)$ did not open in $[\floor{s}, s]$} \right) \\
	&\quad = \frac{\Prob\left( \text{$(i,j)$ opens in $[s, s+ds]$}\right)}{\Prob\left(\text{$(i,j)$ did not open in $[\floor{s}, s]$}\right)} \\
	...
	\begin{equation*}
	\begin{aligned}
	\Prob\left(\NewEdge{[s,s+ds]} \cond \SigmaAlgebra_s \right)
	...
	\begin{equation*}
	\an(s)ds = \Prob\left(\NewEdge{\sds} \cond \SigmaAlgebra_s\right) = (n - \Ineligible{s}) \ps ds.
	\end{equation*}
...
\begin{proof} \label{P: decomp Mn}
	...
	We first have to introduce the quadratic variation of $\Mn$.
	Let $\Pi_n = \{ t_{n,0}, \dots, t_{n,k_n} \}$ be a sequence of partitions of the interval $[0,t]$
	with $|\Pi_n| \rightarrow 0$ as $n \rightarrow \infty$,
	where $|\Pi| := \max_{t_i, t_{i-1} \in \Pi} (t_i - t_{i-1})$ is the mesh of the partition.
	Then, by \cite[Theorem 21.70, p.471]{Klenke.2006}, we have
	\begin{equation*} \label{E: def quadratic variation}
		\sum_{t_i, t_{i-1} \in \Pi_n}\left(\Mn(t_i) - \Mn(t_{i-1})\right)^2 \rightarrow_p [\Mn]_t
	\end{equation*}
	as $n \rightarrow \infty$, where $[\Mn]_t$ is the so-called quadratic variation of $\Mn$ and $\Mn^2 - [\Mn]$ is a martingale.
	By \cite[Remark 21.59, p.468]{Klenke.2006}, 
	for any continuous process of bounded variation $X(t)$ we have $[X]_t = 0$ for all $t$.
...
\begin{proof}
	...
	\begin{aligned}
	\amn(s) &:= \an(s) \left(1 - (s - \floor{s})\p\right) \\
	&= \left(n - s - \Zetan{\ceil{s}} - \Zn(s)\right) \p,
	\end{aligned}
	\end{equation}
	...
	&= \left| \frac{(n - s - \Zetan{\ceil{s}} - \Zn(s))\p}{1-(s - \floor{s})\p} (s - \floor{s})\p) \right| \\
	&= \bigg|\underbrace{\frac{\p n - \p(s + \Zetan{\ceil{s}} + \Zn(s))}{1-(s - \floor{s})\p}}_{\leq 5} 
	\underbrace{(s - \floor{s})\p}_{=\BigO{n^{-1}}} \bigg|\\
	&= \BigO{n^{-1}}.
	\end{aligned}
	\end{equation}
	...
	\begin{equation*}
	\int_{0}^s \left(\amn(u) - 1 + \frac{u}{n} - \frac{\pp}{\n{1}{3}} + \frac{u\pp}{\n{4}{3}} \right) du  
	= \int_{0}^s\left(\amn(u) - 1\right) du + \frac{s^2}{2n} - \frac{s\pp}{\n{1}{3}} + \frac{s^2\pp}{2\n{4}{3}},
	\end{equation*}
	and from \eqref{E: convergence an amn} we know
	\begin{equation*} \label{E: convergence amn An}
	\left|\int_{0}^s(\amn(u) - 1) du - \An(s)\right| = \BigO{\frac{s}{n}}.
	\end{equation*}
	...
	&\quad = \frac{2}{n}  \int_{0}^s \left(1-\min_{w \leq \ceil{u}-1} \Zn(w)  + |\Zn(u)| \right)du + \BigO{\frac{s}{n}}\\ 
	&\quad = \frac{2}{n}  \int_{0}^s \left(|\Zn(u)| - \min_{w \leq \ceil{u}-1} \Zn(w)\right) du + \frac{2s}{n} + \BigO{\frac{s}{n}} \\ 
	&\quad \leq \frac{4s}{n} \max_{u \leq s} |\Zn(u)| + \BigO{\frac{s}{n}},
	\end{aligned}
	\end{equation}
	the last inequality holds since $|\min_{w \leq \ceil{u}-1}\Zn(w)| \leq \max_{u \leq s}|\Zn(u)|$ for $u \leq s$.
\end{proof}
...
\begin{proof}
	...
	\begin{aligned}
	\Prob\left( \sup_{s\leq n^{2/3} s_0} |\Zn(s)| > K \n{1}{3} \right) &= \Prob\left( |\Zn(T_n)| > K\n{1}{3} \right) \\
	...
	\begin{aligned}
	\Exp{|\An(T_n)|} 
	&= \Exp{\left|\int_{0}^{T_n}(\an(s) -1)ds \right|} \\
	...
	\begin{aligned}
	|\an(s)-1| 
	&= \bigg| \an(s) + \amn(s) - \amn(s) + \frac{s}{n} - \frac{\pp}{\n{1}{3}} + \frac{s\pp}{\n{4}{3}} \\
	&\quad	- \frac{s}{n} + \frac{\pp}{\n{1}{3}} - \frac{s\pp}{\n{4}{3}} \bigg| \\
	&\leq \left|\an(s) - \amn(s)\right| + 	\left|\frac{s}{n} - \frac{\pp}{\n{1}{3}} + \frac{s\pp}{\n{4}{3}}\right| \\
	&\quad    + \left|\amn(s) - 1 + \frac{s}{n} - \frac{\pp}{\n{1}{3}} + \frac{s\pp}{\n{4}{3}}\right|
	\end{aligned}
	\end{equation*}
	to evaluate
	\begin{equation*}
	\begin{aligned}
	\Exp{|\An(T_n)|} 
	&\leq \Exp{\int_{0}^{\n{2}{3}s_0} \left|\an(s) - \amn(s)\right|ds} \\
	&\quad + \Exp{\int_{0}^{\n{2}{3}s_0} \left|\frac{s}{n} - \frac{\pp}{\n{1}{3}} + \frac{s\pp}{\n{4}{3}}\right| ds} \\
	&\quad + \Exp{\int_{0}^{\n{2}{3}s_0} \left|\amn(s) - 1 + \frac{s}{n} - \frac{\pp}{\n{1}{3}} + \frac{s\pp}{\n{4}{3}}\right|ds}.
	\end{aligned}
	\end{equation*}
	...
	\begin{equation}
	\begin{aligned}
	&\Exp{\int_{0}^{T_n} \left|\amn(s) - 1 + \frac{s}{n} - \frac{\pp}{\n{1}{3}} + \frac{s\pp}{\n{4}{3}}\right|ds} \\
	...
	\begin{aligned}
	\Exp{|\An(T_n)|} 
	&\leq 4 s_0 K + s_0|\pp|\BigO{\n{1}{3}} + \frac{1}{2}s_0^2\n{1}{3} + s_0|\pp|\n{1}{3} \\
	&\quad + \frac{1}{2}s_0^2|\pp| + s_0\BigO{\n{-1}{3}}.
	\end{aligned}
	\end{equation}
...
\begin{proof}[Proof of Lemma~\ref{L: limit An}] \label{P: limit An}
	...
	\end{aligned}
	\end{equation*}
	To establish the Lemma it is now sufficient to prove
	...
	\begin{equation*}
	\begin{aligned}
	\Prob\left( \n{-2}{3} \supns |\Zn(s)| > \delta\right)
	&= \Prob\left( \n{-1}{3} \supns |\Zn(s)| > \delta \n{1}{3} \right) \\
	&\leq \Prob\left( \n{-1}{3} \supns |\Zn(s)| > K\right) \\
	&< \epsilon,
	\end{aligned}
	\end{equation*}
