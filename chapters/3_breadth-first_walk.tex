...
\section{The deterministic breadth-first walk}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
First, assume $v_{i-1}$ is not the last vertex in it's component.
...
\section{Convergence of the breadth-first walk}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
We denote the stochastic process mapping a realization of $\Gnt$ to the corresponding breadth-first walk by $\znt$.
\fxnote{This sentence is convoluted.}
For $s \geq n$, 
let $\znt(s) := \znt(n)$.
We can now state the main theorem of this chapter,
the convergence of this process to a Brownian motion with drift after rescaling.
...
In the next sections we will now prove Theorem~\ref{T: Z -> W}.
...
The main goal of this chapter will be using the functional central limit theorem for martingales
to prove the convergence in distribution of $\rznt$ to a Brownian motion and subsequently Theorem~\ref{T: Z -> W}.

We analyse the asymptotic behaviour of the processes obtained in the decompositions of the last section
...
\begin{lemma} \label{L: Zn stoch bounded}
	...
	\end{math}
	meaning for all $\epsilon > 0$ exist $K > 0$ and $N \in \Nat$ such that
	\begin{equation} \label{E: stoch bounded}
	\Prob\left( n^{-1/3} \supns |\Zn(s)| > K \right) < \epsilon
	\end{equation}
	for all $n \geq N$.
	\fxnote{Sentence begins with an equation.}
\end{lemma}
