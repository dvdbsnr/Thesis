...
find a way to estimate the probability of encountering one and finally proving the joint convergence of $Z_n$ and the surplus edge counting process.

This chapter will begin with some explanation of the coming theorem.
It will define
\begin{equation}
\zminz(s) = \znt(s) - \min_{u \leq s}\znt(s)
\end{equation}
and explain how the rate of the counting process $\nnt$ at time $s$ is equal to the height of the exploration.
...
the process defined in Chapter~\ref{C: bf-walk} that counted each appearance of a new edge to an eligible vertex.
...
on the some common probability space, such that $\Wt2 \sim \Wt$, $\bar{\mathcal{Z}}^t_i \sim \bar{Z}^t_i$ for all $i \in \Nat$
and $\rznt2 \rightarrow_{a.s.} \Wt2$ as $n \rightarrow \infty$.
...
\begin{aligned}
\Exp{f(\znt, \nnt)} 
&\leq \Exp{f(\Wt, \nnt) \cond \Wt, \Event{C}, \Event{\delta}} + \epsilon + 2M\epsilon   \\
&\leq \Exp{f(\Wt, \rNdis) \cond \Wt} + 2\delta T M + \epsilon  + 2M\epsilon  .
\end{aligned}
\end{equation}
Since neither $\Wt$ nor $\rNdis$ are dependent on $\rznt$ we can drop the events $\Event{C}$ and $\Event{\delta}$ from the conditional expectation.
This inequality holds for all bounded, continuous functions $f$. 
Therefore it holds for $-f$ as well, which implies
\begin{equation}
\begin{aligned}
\Exp{-f(\znt, \nnt)} &\leq \Exp{-f(\Wt, \rNdis) \cond \Wt}+ 2\delta T M  + \epsilon + 2M\epsilon \\
\iff  \Exp{f(\znt, \nnt)} &\geq \Exp{f(\Wt, \rNdis) \cond \Wt} - 2\delta T M  - \epsilon - 2M\epsilon.
\end{aligned}
\end{equation}
In combination with \eqref{E: Estimation step 4} we obtain
\begin{equation} \label{E: Equality of Expectations}
	|\Exp{f(\znt, \nnt)} - \Exp{f(\Wt, \rNdis) \cond \Wt} | \leq 2\delta T M + \epsilon  + 2M\epsilon.
\end{equation}
...
\begin{equation}
\Exp{Y_j} \longrightarrow \int_{s_{j-1}}^{s_j} \Bt(u)du.
\end{equation}
...
\begin{equation} \label{E: Convergence Ndis Nt}
	\Exp{f(\Wt, \Ndis) \cond \Wt} \longrightarrow \Exp{f(\Wt, \Nt) \cond \Wt}.
\end{equation}
