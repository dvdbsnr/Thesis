...
We will make use of the following theorem to link convergence in distribution of random variables to almost sure convergence of random variables on the same probability space.
\begin{theorem}[Skorohod, {\cite[Theorem 3.30, p.56]{Kallenberg.1997}}] \label{T: Skorohod}
...
we quickly observe that the notion of convergence known in $\CT$,
the space of continuous functions on $[0,T]$ where
\begin{equation*}
	f_n \rightarrow f \iff \sup_{t \in [0,T]} |f_n(t) - f(t)| \rightarrow 0,
\end{equation*}
...
Here the first condition provides a general boundedness of the set and can be found in the $\CT$ Arzelà-Ascoli theorem as well, 
while the second condition is unique to the space $\DT$.
...
We conclude this preliminary chapter by presenting Girsanov's theorem,
