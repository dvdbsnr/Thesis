...
\begin{lemma} \label{L: Deterministic Lemma}
	...
	Then $\Xin \rightarrow_d \Xi$ as $n \rightarrow \infty$.
\end{lemma}
...
and $(\lnx, \rnx- \lnx) \in \Xin$.

Define $\lnx := \argmin_{u \leq x} f_n(u)$.
...
So $\lnx \in [l-\epsilon, l+\epsilon]$ and
$\lnx = \min_{u \leq \lnx} f_n(u)$.
...
To recap: We have shown that for any excursion $(l,r)$ of $f$ there are two points $\lnx$ and $\rnx$, very close to $l$ and $r$ respectively,
satisfying condition \eqref{E: fn cond 4} with no other eligible points in between.
The question remains whether these points must be elements of $\fnPoints$ 
or if it's possible to skip large excursions of $f$ by missing elements on $\fnPoints$.
First of all, consider that \eqref{E: fn cond 3} ensures there is no last element of $\fnPoints$.
Any excursion of $f_n$ started will eventually end.
For any excursion $(l,r)$ of $f$, if $\lnx \in \fnPoints$ there will be some point greater or equal to $\rnx$ present in $\fnPoints$,
so the excursion of $f$ is matched by some excursion of similar or greater length.
The only problem that could arise is $\fnPoints$ skipping the end of one
and the start of another large excursion of $f$,
or even skipping multiple large excursions of $f$.

Consider two consecutive large excursions of $f$, $(l_1, r_1)$ and $(l_2, r_2)$
and the space between $r_1$ and $l_2$.
Suppose there is either no element of $\fnPoints$ in $[r_1-\epsilon, l_2+\epsilon]$ for all $\epsilon>0$.
...
$|f_n(l^*) - f_n(r^*)|> \delta$ for all $l^*$ and $r^*$ in the respective $\epsilon$-neighbourhoods around $l_1$ and $r_2$.
Any possible point of $\fnPoints$ before $l_1$ will only be higher, any point after $r_2$ will only be lower.
This is a contradiction to \eqref{E: fn cond 5}.
So there must be at least a point of $\fnPoints$ in between these two large excursions, no excursion can be skipped.
\fxfatal{Fix everything about this. Please. Fuck} 


Additionally, we have shown that any point satisfying \eqref{E: fn cond 4} must lay in some $\epsilon$-neighbourhood around a start- or endpoint of some excursion of $f$.
\fxfatal{Show this!}
We will now show that conditions \eqref{E: fn cond 3} and \eqref{fn cond 5} assure us 
that the set $\fnPoints$ must contain points in some $\epsilon$-neighbourhood of $l$ and $r$.
First of all, \eqref{E: fn cond 3} ensures that there is no last element in any sequence $\fnPoints$.
Our only concern is $\fnPoints$ skipping an excursion.
Consider again the fixed pair $(l,r)$.
Suppose there is no element of $\fnPoints$ in $[l-\epsilon, l+\epsilon] $
